...
from partaj.core.indexers import NotesIndexer, ReferralsIndexer
...
def perform_create_empty_index(indexable, logger=None):
    """
    Create a new index in ElasticSearch from an indexable instance.
    """
    # Create a new index name, suffixing its name with a timestamp
...
def regenerate_note_index(logger=None):
    """
    Create new index for our note index and replace possible existing index with
    a new one only once it has successfully built it.
    """
    # Get existing index
    try:
        existing_index = ES_INDICES_CLIENT.get_alias(NotesIndexer.index_name)
    except NotFoundError:
        # Provide a fallback empty list so we don't have to check for its existence later on
        existing_index = {}

    # Create a new index for this module
    # NB: we're mapping perform_create_index which produces side-effects
    indices_to_create = zip(
        list(map(lambda ix: perform_create_empty_index(ix, logger), [NotesIndexer])),
        [NotesIndexer],
    )
    ...
    # Replace the old index with the new one in 1 atomic operation to avoid outage
    def perform_aliases_update():
        try:
            ES_INDICES_CLIENT.update_aliases(
                dict(actions=actions_to_create_aliases + actions_to_delete_aliases)
...
def regenerate_referral_index(logger=None):
    """
    Create new index for our referral index and replace possible existing index with
    a new one only once it has successfully built it.
    """
    # Get existing index
    try:
        existing_index = ES_INDICES_CLIENT.get_alias(ReferralsIndexer.index_name)
    except NotFoundError:
        # Provide a fallback empty list so we don't have to check for its existence later on
        existing_index = {}
    ...
    indices_to_create = zip(
        list(
            map(lambda ix: perform_create_empty_index(ix, logger), [ReferralsIndexer])
        ),
        [ReferralsIndexer],
    )
