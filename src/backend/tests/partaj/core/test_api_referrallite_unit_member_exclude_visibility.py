...
class ReferralLiteUnitMemberExcludeVisibilityDashboardApiTestCase(TestCase):
    """
    ...
    def test_list_referrals_by_unit_member(self):
        ...
        topic = factories.TopicFactory(unit=factories.UnitFactory(
            member_role_access=MemberRoleAccess.RESTRICTED,
        ))
        ...
        self.assertEqual(member_unit_response.json()["all"]["count"], 5)
        ...
        self.assertEqual(owner_unit_response.json()["all"]["count"], 5)
    ...
    def test_visibility_from_received_to_splitting_in_unit_received_not_visible(self):
        ...
        self.assertEqual(owner_unit_response.status_code, 200)
        self.assertEqual(owner_unit_response.json()["all"]["count"], 1)
        ...
        self.assertEqual(random_member_unit_response.json()["all"]["count"], 1)
        ...
        self.assertEqual(member_unit_response.status_code, 200)
        self.assertEqual(member_unit_response.json()["all"]["count"], 1)
