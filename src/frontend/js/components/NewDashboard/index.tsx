...
export const NewDashboard: React.FC = () => {
  const columns: Column[] = [
    { name: 'id', label: '#' },
    { name: 'created_at', label: 'Créé le', render: (item: ReferralLite) => formatDate(new Date(item.created_at)) },
    { name: 'due_date', label: 'Echéance', render: (item: ReferralLite) => formatDate(new Date(item.due_date)) },
    { name: 'object', label: 'Titre' },
    { name: 'requesters', label: 'Unités des demandeurs', render: (item: ReferralLite) => item.requesters.map(requester => requester.unit_name).join(", ") },
    {
      name: 'assignees',
      label: "Affectation(s)",
      render: (item: ReferralLite) => item.assignees.map(assignee => assignee.first_name + " " + assignee.last_name).join(", ")
    },
    {
      name: 'state',
      label: 'Statut',
    ...
    },
    {
      name: 'published_date',
      label: 'Date de rendu',
      render: (item: ReferralLite) => item.published_date ? formatDate(new Date(item.published_date)) : "",
    }
  ]
  ...
  const [referralState, setReferralState] = useState<ReferralState | 'all'>((urlParams.get('state') as ReferralState) || 'all');
  ...
  const filters = useMemo(() => {
    const filters: UseReferralLitesParams & { sort?: string, sort_dir?: string } = {};

    const getSortKey = (column: string): string => {
      switch (column) {
        case "id": return "case_number";
        case "object": return "object.keyword";
        case "requesters": return "users_unit_name_sorting";
        case "assignees": return "assignees_sorting";
        case "state": return "state_number";
        default: return column;
      }
    }

    if (search) filters.query = search;
    if (referralState && referralState !== 'all') filters.state = [referralState];
    if (userId) filters.assignee = [userId];
    if (unitId) filters.unit = [unitId];
    if (dateRange && dateRange.from) filters.due_date_after = format(dateRange.from, "yyyy-MM-dd");
    if (dateRange && dateRange.to) filters.due_date_before = format(dateRange.to, "yyyy-MM-dd");
    if (sortColumn) {
    ...
    return filters;
  }, [search, referralState, userId, unitId, dateRange, sortColumn, sortDirection]);
  ...
  const renderActiveFilters = () => {
    ...
    if (userId) {
      const selectedUser = userOptions?.find(option => option.value === userId.toString());
      activeFilters.push(
        <ClickableBadge
          key="user"
          onClose={() => {
    ...
    if (unitId) {
      const selectedUnit = unitOptions?.find(option => option.value === unitId.toString());
      activeFilters.push(
        <ClickableBadge
          key="unit"
          onClose={() => {
  ...
  return (
    <div className="p-4">
      ...
      <Tabs
        defaultValue="all"
        onValueChange={(value) => handleReferralStateChange(value as ReferralState | 'all')}
                ...
                >
                  <div className='flex items-center'>
                    ...
                    {sortColumn === column.name &&
                      (sortDirection === SortDirection.Asc ? <ChevronUp className='ml-1 h-4 w-4' /> : <ChevronDown className='ml-1 h-4 w-4' />)}
                  </div>
                </TableHead>
              ))}
            </TableRow>
