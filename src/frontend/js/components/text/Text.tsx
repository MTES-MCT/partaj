...
export const Text: React.FC<{ type?: TextType; className?: string }> = ({
  ...
  switch (type) {
    case TextType.PARAGRAPH_NORMAL:
      return (
        <p className={`text-base font-light ${className}`}> {children}</p>
      );
    ...
    case TextType.PARAGRAPH_DESCRIPTION:
      return (
        <p className={`text-sm font-light text-grey-600 ${className}`}> {children}</p>
      );
    case TextType.SPAN_DESCRIPTION:
      return (
        <span className={`text-sm text-grey-600 font-light ${className}`}> {children}</span>
      );
    default:
      return (
